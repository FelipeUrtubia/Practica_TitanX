...
function sound_analyzer_v12(_topFrame,_libraryPath,_codebasePath, _inputParameters) {
...
_stringProperties.textopenpipe = "Open pipe with length = ";
_stringProperties.textclosepipe = "Closed pipe with length = ";
_stringProperties.textinstruct = "Blow an open-open or open-closed \nend pipe to analyze the sound";
  var __pagesEnabled = [];
  ...
  var maximumXs; // EjsS Model.Variables.Vars.maximumXs
  var minimumX; // EjsS Model.Variables.Vars.minimumX
  ...
  var howmanypeaks; // EjsS Model.Variables.Vars.howmanypeaks
  ...
  var textshow; // EjsS Model.Variables.lookang.textshow
  var textinstruct; // EjsS Model.Variables.lookang.textinstruct
  var textclosepipe; // EjsS Model.Variables.lookang.textclosepipe
  var textopenpipe; // EjsS Model.Variables.lookang.textopenpipe
  ...
  _model._userSerialize = function() {
    return {
      ...
      textclosepipe : textclosepipe,
      textopenpipe : textopenpipe
    };
  };
  ...
  _model._readParameters = function(json) {
    ...
    if(typeof json.textinstruct != "undefined") textinstruct = json.textinstruct;
    if(typeof json.textclosepipe != "undefined") textclosepipe = json.textclosepipe;
    if(typeof json.textopenpipe != "undefined") textopenpipe = json.textopenpipe;
  };
  ...
  _model.addToReset(function() {
    ...
    __pagesEnabled["yellow"] = true;
    __pagesEnabled["evolution"] = true;
    __pagesEnabled["findlong"] = true;
    __pagesEnabled["findlong 2"] = false;
    __pagesEnabled["resetpeakevery3sec"] = true;
  });

  _model.addToReset(function() {
    audioCtx = []; // EjsS Model.Variables.Vars.audioCtx
    analyser = []; // EjsS Model.Variables.Vars.analyser
    ...
    maxDecibels = -20; // EjsS Model.Variables.Vars.maxDecibels
    minDecibels = -80; // EjsS Model.Variables.Vars.minDecibels
    maximumX = 2000; // EjsS Model.Variables.Vars.maximumX
    maximumXs = 2000; // EjsS Model.Variables.Vars.maximumXs
    minimumX = 100; // EjsS Model.Variables.Vars.minimumX
    peakShadow = true; // EjsS Model.Variables.Vars.peakShadow
    areaShadow = 20; // EjsS Model.Variables.Vars.areaShadow
    xlabel = []; // EjsS Model.Variables.Vars.xlabel
    ylabel = []; // EjsS Model.Variables.Vars.ylabel
    tlabel = []; // EjsS Model.Variables.Vars.tlabel
    howmanypeaks = 3; // EjsS Model.Variables.Vars.howmanypeaks
  });
  ...
  _model.addToReset(function() {
    ...
    textshow = ""; // EjsS Model.Variables.lookang.textshow
    textinstruct = "Blow an open-open or open-closed \nend pipe to analyze the sound"; // EjsS Model.Variables.lookang.textinstruct
    textclosepipe = "Closed pipe with length = "; // EjsS Model.Variables.lookang.textclosepipe
    textopenpipe = "Open pipe with length = "; // EjsS Model.Variables.lookang.textopenpipe
  });
  ...
  _model.addToInitialization(function() {
    if (!__pagesEnabled["yellow"]) return;
    //"Sound Analyzer"+"\nMax Freq="+maximumX+ " Hz"  // > Initialization.yellow:1
    //"baseFreq="+baseFreq.toFixed(1)+" Hz"  // > Initialization.yellow:2
    //  // > Initialization.yellow:3
    textshow = textinstruct;  // > Initialization.yellow:4
  });
  ...
  _model.addToEvolution(function() {
    ...
    if (vals.length != 0) {   // > Evolution.findlong:3
     ...
     textshow = textopenpipe+ Math.round(vals[1] * 100) / 100 + " m" ;  // > Evolution.findlong:5
     //_view.text.setText(textopenpipe + Math.round(vals[1] * 100) / 100 + " m") ;  // > Evolution.findlong:6
     }  // > Evolution.findlong:7
     else if (vals[0] == false) {// vals[0] is false  // > Evolution.findlong:8
      textshow = textclosepipe+ Math.round(vals[1] * 100) / 100  + " m";  // > Evolution.findlong:9
     // _view.text.setText(textclosepipe + Math.round(vals[1] * 100) / 100  + " m");  // > Evolution.findlong:10
    }  // > Evolution.findlong:11
    else { // add by lookang to address text locked in wrong state  // > Evolution.findlong:12
       textshow=textinstruct;  // > Evolution.findlong:13
     //  _view.text.setText(textinstruct);  // > Evolution.findlong:14
      }  // > Evolution.findlong:15
      }  // > Evolution.findlong:16
  });

  _model.addToEvolution(function() {
    if (!__pagesEnabled["findlong 2"]) return;
    // find long  // > Evolution.findlong 2:1
    var vals = findLong(); // return true,0.30911111 for open pipe  // > Evolution.findlong 2:2
    if (vals.length != 0) {   // > Evolution.findlong 2:3
     if (vals[0] == true) // vals[0] is true  // > Evolution.findlong 2:4
      _view.text.setText("Open pipe with length = " + Math.round(vals[1] * 100) / 100 + " m") ;  // > Evolution.findlong 2:5
     else if (vals[0] == false) // vals[0] is false  // > Evolution.findlong 2:6
      _view.text.setText("Closed pipe with length = " +  Math.round(vals[1] * 100) / 100  + " m");  // > Evolution.findlong 2:7
    }  // > Evolution.findlong 2:8
    else { // add by lookang to address text locked in wrong state  // > Evolution.findlong 2:9
       _view.text.setText("Blow an open-open or open-closed \nend pipe to analyze the sound");  // > Evolution.findlong 2:10
      }  // > Evolution.findlong 2:11
  });
  ...
  function _selectView(_viewNumber) {
    _view = null;
    _view = new sound_analyzer_v12_View(_topFrame,_viewNumber,_libraryPath,_codebasePath);
  ...
  */;
  ...
  //ylabel[elementinteracted]=_info._point[1];
          ...
          _view.text.linkProperty("Text",  function() { return textshow; }, function(_v) { textshow = _v; } ); // HtmlView Page linking property 'Text' for element 'text'
          ...
          _view.xaxistext.linkProperty("X",  function() { return maximumX-10; } ); // HtmlView Page linking property 'X' for element 'xaxistext'
          _view.xaxistext.linkProperty("Y",  function() { return minDecibels+5; } ); // HtmlView Page linking property 'Y' for element 'xaxistext'
          _view.xaxistext.linkProperty("Font",  function() { return fontaxis; }, function(_v) { fontaxis = _v; } ); // HtmlView Page linking property 'Font' for element 'xaxistext'
          ...
          _view.axisself.linkProperty("Y",  function() { return -80+1; } ); // HtmlView Page linking property 'Y' for element 'axisself'
...
function sound_analyzer_v12_View (_topFrame,_viewNumber,_libraryPath,_codebasePath) {
  var _view;
  switch(_viewNumber) {
    case -10 : break; // make Lint happy
    default :
    case 0: _view = sound_analyzer_v12_View_0 (_topFrame); break;
  } // end of switch
...
function sound_analyzer_v12_View_0 (_topFrame) {
  ...
  _view._reset = function() {
    ...
    _view._addElement(EJSS_INTERFACE.panel,"configPanel", _view.singlePlotPanel) // EJsS HtmlView.HtmlView Page: declaration of element 'configPanel'
      .setProperty("Display","none") // EJsS HtmlView.HtmlView Page: setting property 'Display' for element 'configPanel'
      ;
    ...
    _view._addElement(EJSS_INTERFACE.checkBox,"peakShadow", _view.controlPanel) // EJsS HtmlView.HtmlView Page: declaration of element 'peakShadow'
      ...
      .setProperty("Display","none") // EJsS HtmlView.HtmlView Page: setting property 'Display' for element 'peakShadow'
      ;
    ...
    _view._addElement(EJSS_DRAWING2D.text,"xaxistext", _view.plottingPanel) // EJsS HtmlView.HtmlView Page: declaration of element 'xaxistext'
      .setProperty("RelativePosition","SOUTH_EAST") // EJsS HtmlView.HtmlView Page: setting property 'RelativePosition' for element 'xaxistext'
      .setProperty("Text","Frequency / Hz") // EJsS HtmlView.HtmlView Page: setting property 'Text' for element 'xaxistext'
      ;
    ...
    _view._addElement(EJSS_DRAWING2D.textSet,"axisself", _view.plottingPanel) // EJsS HtmlView.HtmlView Page: declaration of element 'axisself'
      .setProperty("NumberOfElements",3) // EJsS HtmlView.HtmlView Page: setting property 'NumberOfElements' for element 'axisself'
    ...
    _view._addElement(EJSS_INTERFACE.panel,"executionPanel", _view.singlePlotPanel) // EJsS HtmlView.HtmlView Page: declaration of element 'executionPanel'
      ...
      .setProperty("CSS",{   "position" : "absolute",   "top" : "0%",    "margin-right":"0px",    "left":"0%" }) // EJsS HtmlView.HtmlView Page: setting property 'CSS' for element 'executionPanel'
      ;
    ...
    _view._addElement(EJSS_INTERFACE.twoStateButton,"runPauseButton", _view.executionPanel) // EJsS HtmlView.HtmlView Page: declaration of element 'runPauseButton'
      .setProperty("Height","5vmax") // EJsS HtmlView.HtmlView Page: setting property 'Height' for element 'runPauseButton'
      .setProperty("Width","100%") // EJsS HtmlView.HtmlView Page: setting property 'Width' for element 'runPauseButton'
      .setProperty("TextOn","Press here to start capturing ►") // EJsS HtmlView.HtmlView Page: setting property 'TextOn' for element 'runPauseButton'
      .setProperty("Tooltip","Play/Pause") // EJsS HtmlView.HtmlView Page: setting property 'Tooltip' for element 'runPauseButton'
      .setProperty("TextOff","Press here to pause capturing ❚❚") // EJsS HtmlView.HtmlView Page: setting property 'TextOff' for element 'runPauseButton'
      ;
    ...
    _view._addElement(EJSS_INTERFACE.button,"resetButton", _view.executionPanel) // EJsS HtmlView.HtmlView Page: declaration of element 'resetButton'
      ...
      .setProperty("Display","none") // EJsS HtmlView.HtmlView Page: setting property 'Display' for element 'resetButton'
      ;
